...
$selectedYear = isset($_GET['selectedYear']) ? $_GET['selectedYear'] : date('Y'); // Get selected year from URL, default to current year
...
// Get the selected year from the URL, default to the current year if not selected
...
for ($i = 1; $i <= 12; $i++) {
    // Query to get the total renters for each month, based on `from_date` and selected year
    $query = "SELECT COUNT(*) AS total FROM booking WHERE MONTH(from_date) = ? AND YEAR(from_date) = ?";
    
    // Prepare the SQL statement
    if ($stmt = $conn->prepare($query)) {
        // Bind the parameters to the query
        $stmt->bind_param("ii", $i, $selectedYear);
...
// Query to get available years for filtering
$sql = "SELECT DISTINCT YEAR(from_date) AS year FROM booking ORDER BY year DESC";
$result = $conn->query($sql);

// Check if the query succeeded
if ($result === false) {
    die('Error in query execution: ' . $conn->error); // Stop execution and display error if query fails
}
...
$sql = "SELECT image, car_name, COUNT(*) as rental_count
        ...
        LIMIT 3"; // Modify LIMIT if you want more cars
...
$queryCancel = "SELECT COUNT(*) FROM book_cancel WHERE status = 0"; // Count of canceled bookings
...
<head>
	...
	<style>
		...
		.car-status-box {
			...
			width: 270px; /* Ensure it stays within the container */
...
<body>
				...
				<div class="report-left">
					...
					<div class="report-box full-width">
						<div class="box-content" style="position: relative;">
							...
							<!-- Form to filter by year -->
							<form method="GET" action="">
								<select id="yearSelect" name="selectedYear" style="position: absolute; top: 0; right: 20px; padding: 8px; border: 1px solid #ddd; border-radius: 4px;" onchange="this.form.submit()">
									<?php
									// Generate options for each year
									foreach ($years as $year) {
										echo "<option value='$year' " . ($year == $selectedYear ? 'selected' : '') . ">$year</option>";
									...
									<div class="car-status-content">
										...
										<div class="car-details">
											<div class="car-info">
												...
												<p class="person-count bx bx-user" style="font-size: 20px; margin-left: 5px;"> 
													<?php echo $car['rental_count']; ?> person<?php echo $car['rental_count'] > 1 ? 's' : ''; ?>
												</p>
											</div>
                ...
                y: {
                    ...
                    ticks: {
                        callback: function(value) {
                            return '₱' + value.toLocaleString(); // Add "₱" symbol
                        }
                    },
                    title: {
                        display: true,
                ...
                tooltip: {
                    ...
                    callbacks: {
                        label: function(tooltipItem) {
                            return '₱' + tooltipItem.raw.toLocaleString(); // Format tooltip as Philippine pesos with "₱"
                        }
                    }
                }
            }
    ...
    document.getElementById('yearSelect').addEventListener('change', function() {
        ...
        window.location.href = '?selectedYear=' + selectedYear; // Reload page with the new selected year
    });
...
// Listen for year change and reload chart with new data
document.getElementById('yearSelect').addEventListener('change', function() {
    const selectedYear = this.value;
    // Reload the page with the selected year (form submission)

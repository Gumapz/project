							...
							foreach ($result as $r) {
								...
								<tr class="gradeX">
									...
									<td style="text-align: center;">
										...
										echo $r['payment_status'] == 1 ? 'Paid' : 'Not Paid'; 
										?>
									</td>
									<td style="text-align: center;">
										<?php 
											if ($r['returned']) {
											 // Display pickup if it exists
											echo htmlspecialchars($r['returned']);
											} else {
												echo htmlspecialchars($r['pickup']);// Otherwise display returned
											}
										?>
									</td>
									...
									<td style="text-align: center;">
										<?php 
										if ($r['returned'] === 'Returned') { // Check if the car is returned
											echo ($r['penalty_amount'] && $r['penalty_amount'] > 0) 
												? 'PHP ' . htmlspecialchars($r['penalty_amount']) 
												: 'No Penalty';
										} else {
										}
										?>
